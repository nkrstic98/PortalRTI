...
export class SubjectExamsComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<FileInfo[]>) {
    moveItemInArray(this.subject.fajlovi_ispit, event.previousIndex, event.currentIndex);
    console.log(this.subject.fajlovi_ispit);
  }

  changeFileOrder() {
    this.alertService.clear();

    this.subjectService.reorderDocuments(this.subject.fajlovi_ispit, this.route.snapshot.params['sifra'], 'fajlovi_ispit')
      .pipe(first())
      .subscribe({
        next: value => {
